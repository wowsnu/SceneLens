...
주장과 화면이 반대로 간다. 그래서 렌즈를 **동시에 켤 수 있는 오버레이**로
바꾸고, 여러 렌즈가 같은 대상을 볼 때 그것이 `Stack`으로 보이게 한다
(`LENS_STACK_UI.md`). 부수적으로 에이전트를 캐릭터화하지 않아도 된다 —
...
**인터페이스 개편에서도 유지된다 (2026-08-25).** Lens Stack에 서사를 넣지
않는다. 스택은 mise/camera/editing 셋이다 — 위 이유가 그대로 적용된다
(`LENS_STACK_UI.md` 5장).
