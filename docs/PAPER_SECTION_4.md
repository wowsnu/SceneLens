...
주장과 화면이 반대로 간다. 그래서 세 렌즈를 스토리보드와 가로축을 공유하는
**트랙**으로 나란히 두어, 여러 렌즈가 같은 지점을 짚은 것이 **수직 정렬**로
보이게 한다 — 관계가 별도 카드의 설명이 아니라 배치 자체가 된다
(`LENS_TRACKS_UI.md`). 부수적으로 에이전트를 캐릭터화하지 않아도 된다 —
사용자가 경험하는 개념이 대화 상대가 아니라 artifact를 통과하는 렌즈다.
...
**서사는 Decision Board에 두지 않는다 (2026-08-13 결정).**

`PERSPECTIVES = [NARRATIVE_AGENT, ...CREATIVE_LENSES]` — 코드가 이미 서사를
셋 위에 두고 있다. 렌즈 레인에 넣으면 이 위계가 평평해진다.

더 중요한 이유는 시점이다. **Decision Board는 그려진 패널을 검토하는
화면**이라, 거기서 "이 컷은 필요 없다"는 말을 들으면 이미 그린 것을 버려야
한다. 서사가 짚는 것은 컷 구성 자체이므로 **그리기 전에 짚어야 싸다.**

그래서 컷 플랜 단계의 서사 레일에 `컷 구성 점검`을 붙였다
(`narrative_check.py`, `/api/narrative/check`). 규칙 4개로 컷 플랜 전체를
훑고 결과를 그 레일에 띄운다. 레일은 이미 `cutplan` 단계에서 기본으로
열려 있었다 (`StoryboardView.jsx:942`).

| | 언제 | 무엇을 보고 |
|---|---|---|
| 서사 **제안** (`/narrative/suggest`) | 감독이 요청할 때 | 그 Beat의 대본 줄 |
| 서사 **점검** (`/narrative/check`) | 컷 플랜 단계, 버튼 | 컷 플랜 전체 |
| 나머지 세 렌즈 | 생성 후 | 렌더된 패널 |

**인터페이스 개편에서도 유지된다 (2026-08-25).** Lens Tracks에 서사를 넣지
않는다. 트랙은 mise/camera/editing 셋이다 — 위 이유가 그대로 적용된다
(`LENS_TRACKS_UI.md` 9장).

**논문 서술.** "4 lenses"로 쓰되, 서사는 나머지 셋과 **같은 자리에서 같은
때에 작동하지 않는다**고 밝혀야 정확하다. 셋은 생성된 화면을 진단하고,
서사는 무엇을 그릴지가 정해지기 전에 판단한다. 이것이 4.4의 "문제가
존재하는 층위에서 개입한다"와 같은 논리다 — 서사 문제의 층위는 컷 플랜이지
패널이 아니다.
