...
### 관객 에이전트 페르소나 — 설계 논의만 (2026-08-13)
...
**정해진 방향**
- 고정 카드가 아니라 감독이 선언하는 조건에서 만든다 (CLOChat 방향).
  작품에서 유도할 수 없다 — 같은 대본이 영화제용일 수도 온라인용일 수도
  있고, 그건 감독의 선택이지 대본에 적혀 있지 않다.
- 질문은 하나로 좁힌다: **"이 장면에서 특히 확인하고 싶은 읽기는 누구의
  것인가?"**
  - 처음 보는 관객 / 영화에 익숙한 관객 / 이 이야기와 가까운 관객
  - 세 번째는 "무엇에 가까운지" 한 줄을 감독이 쓴다. 앞의 둘은 영화
    문법과의 거리이고 세 번째는 소재와의 거리라, 축이 다르다.
