...
**진단은 문제의 층위를 밝힌다.** DG2는 원인이 속성·컷의 존재·컷 간 관계·씬 범위
중 어디인지 진단하고 해당 층위에서 개입할 것을 요구한다. `PROBLEM_LAYERS`가 그
넷이고, 촬영·편집의 모든 진단이 `layer`를 갖는다.

| 층위 | 예 | 개입 수단 |
|---|---|---|
| 속성 | 점프컷 위험 | 표의 셀 편집 |
| 컷의 존재 | 리액션 없음, 압축됨, Beat에 컷 없음 | 행 `+` / 삭제 |
| 컷 간 관계 | 같은 크기 연속, 내용 중복 | **없음 — 이음새 미구현** |
| 씬 범위 | 공간 세우는 컷 없음 | 컷 추가 + `sceneState` |

층위를 표시해 두면 **개입 수단이 없는 층위가 화면에 드러난다.** 컷 간 관계
진단을 보고 누를 것이 없다는 것이 곧 이음새 상태가 필요해지는 시점이다 — 미리
만들지 않고 그때 붙인다.
