...
**선언 전부를 그림 앞에 두지 않는다.** 초안 생성을 바꾸는 것만 게이트에
올린다(`affectsDraft`). 나머지는 Shot Inspector에서 실제 패널을 보고 판정한다.
전부를 앞으로 몰면 아무것도 보지 못한 상태에서 판정을 강요하게 되고 이는 P4와
정면으로 부딪힌다. 게이트를 없애지 않는 이유는 DG1 → DG3 연결이 무언가는
생성보다 앞설 것을 요구하기 때문이다 — 문제는 위치가 아니라 분량이었다.

현재 게이트에 오르는 것은 씬 전체 위임(의상·미술)과 인물 외형 일관성 셋이다.
...
**이미지 밖 채널은 패널 위에 그려진다.** 목록의 글자로만 두면 "그림 밖 채널에
기록한다"가 성립하지 않는다 — 화살표는 화살표여야 의도적으로 남긴 것이 누락으로
보이지 않는다. `OFFIMAGE_CHANNELS`의 각 항목이 `mark`를 갖는다.

| mark | 표현 | 채널 |
|---|---|---|
| `arrow` | 패널 위 방향 화살표 | 움직임 화살표, 카메라 이동 |
| `corner` | 모서리 배지 | 타임코드 |
| `note` | 패널 아래 텍스트 | 액팅 메모, 카피 |

`buildPanelMarks(offImage)`가 선언을 마크로 바꾼다. 방향 문구에서 각도를 읽지
못하면(`readArrowAngle`이 `null`) 화살표를 그리지 않고 메모로 떨어뜨린다 —
방향을 모르면서 화살표를 그리면 거짓말이 된다.
