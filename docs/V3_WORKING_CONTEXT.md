...
- 연출 분석 규칙: [`DIRECTING_RULES.md`](./DIRECTING_RULES.md)
...
**2026-08-10 단순화.** 책임 분류를 모든 요소에 요구하지 않는다. 기존 선언과
제약 해제 통로는 인스펙터의 `고급 표시 설정`에 접어 보존하고, 기본 작업은 패널
아래의 `카메라 이동`과 `메모` 도구로 한다. 카메라 이동은 패널 위에 방향을 직접
그린 뒤 PAN/TILT/TRACK/DOLLY/ZOOM을 붙인다. 연출 분석의 수정 방향에서도
`카메라 화살표`, `메모로 남기기`, `이미지 수정/컷 구성 열기`로 해당 도구에 바로
이동한다. 즉 책임 상태를 먼저 관리하는 대신 필요한 순간에 실제 스토리보드 표기로
남긴다.
...
#### 대본 형식과 연구 범위 (2026-08-09)

**대본은 주어진 것에서 시작한다.** 시나리오 저작은 스토리보드 연구의 범위가
아니고, 실험도 완성된 대본으로 진행한다(참가자가 창작까지 하면 조건이 통제되지
않고 시간도 그쪽에 쏠린다). 지함 formative의 순서도 `감독이 대본을 쓴다 →
줄콘티 → 그림 콘티`로, 대본 쓰기는 스토리보드 이전이다.

제거: 형식 변환(줄글→시나리오 포맷), Beat만 나누는 별도 단계, `ROUGH_SCREENPLAY`.
유지: 줄 인라인 수정, Beat 경계 조정 — DG3에서 발견된 어긋남이 대본까지
되돌아갈 수 있어야 순환이 성립한다.

**대신 이야기 → 씬·비트 구조는 AI가 돕는다**(`requestStoryStructure`). 참가자가
대본을 갖고 오지 않아도 되게 하되, 창작 부담은 짧은 이야기 한 덩어리까지만
지운다. 이것은 시나리오 저작이 아니라 **스토리보드가 필요로 하는 단위를 드러내는
일**이다 — 컷을 나누려면 씬과 비트가 먼저 있어야 한다.

- 장소가 바뀌면 씬을 나눈다. 다만 언급과 이동은 다르다 — "승강장 사람들이
  위험해진다"는 관제실 안에서 하는 말이므로, 이동 동사가 함께 있을 때만 나눈다.
- 국면 전환어(`근데`, `그러다`…)로 시작하면 비트를 나눈다.
- 이야기 말투를 서술로 옮긴다(`toNarrative`): `들어감 → 들어간다`,
  `달려듦 → 달려든다`, `총괄임 → 총괄이다`. 한국어 명사형 종결은 받침으로
  갈리므로 종성을 분해해 처리한다(ㅁ=16, ㄻ=10).
- **내용은 더하지 않는다.** 이야기에 있는 것만 문장으로 푼다 — 사용자가 쓰지
  않은 것이 대본에 들어가면 출처를 알 수 없게 된다.

제안으로 두고 사용자가 확인해야 적용된다(DG1 P2). 확인 전까지 원문은 그대로다.

**실제 모델이 붙었다** (2026-08-09). `POST /api/story/structure` —
FastAPI 백엔드(`backend/app/services/story_structure.py`), gpt-5.4-nano,
structured output. 실패하면 규칙 기반 mock으로 떨어져 작업이 멈추지 않고,
그 경우 화면에 "AI 호출에 실패해 규칙 기반으로 나눴습니다"라고 밝힌다 —
규칙 기반 결과를 모델이 만든 것처럼 보이게 두지 않는다.

첫 호출에서 mock으로는 드러날 수 없던 것들이 나왔다. 프롬프트가 그것을
막도록 고쳐졌다.

| 모델이 한 것 | 왜 문제인가 |
|---|---|
| 없는 자세·소리·반응을 지어냄 | 사용자가 쓰지 않은 것이 대본에 들어간다 |
| 한 방을 두 씬으로 나눔 | 씬은 장소가 실제로 바뀔 때만 나뉜다 |
| 줄마다 비트 하나 | 비트가 국면 단위가 아니게 된다 |
| "누르면 위험해진다" | 조건은 화면이 담을 수 없다 |

규칙을 산문으로 적는 것만으로는 부족했다. 이 씬에서 실제로 나온 거절 예시를
채택 예시와 나란히 보여주니 지켜졌다.

**대사를 두지 않는다.** 정지 이미지가 담을 수 없고, 비교 대상(storyboarder.ai)도
대사를 다루지 않으며, 평가 목적도 아니다. 실제로 `buildCutPrompt`는 이미 대사를
`"..."라고 말하는 순간이다`로 감싸 표정·자세로 번역하고 있었다 — 처음부터 그렇게
쓰는 편이 맞다.
...
줄 종류도 `scene-heading` / `action` 둘만 남았다. `character`, `dialogue`,
`parenthetical`, `transition`은 제거했다.

컷 분해는 **대사 화자 → 행동 단위**로 바뀌었다. 서술 문장 하나가 컷 하나이고,
그 문장에 나오는 인물만 그 컷에 들어간다. 둘이 함께면 `Medium · 관계`, 혼자면
`Bust · 행동 강조`. 예제 기준 12컷 → 19컷.

**씬이 화면에 보인다.** 대본 뷰와 컷 표 모두 씬 경계를 그린다 —
`Scene 1 · 관제실, 밤`. Beat보다 큰 단위이므로 위에, 더 크게 둔다.
예제 대본도 씬 둘(관제실 → 승강장)로 늘려 위계가 실제로 보이게 했다.

`inferSceneContext`는 이제 **씬 단위로** 호출된다. 대본 전체에서 한 번 뽑으면
두 번째 씬의 컷이 첫 씬의 장소를 물려받는다 — 승강장 컷의 장소가 `관제실`로
찍히던 버그가 그것이다.

**씬도 접힌다.** Beat와 별개 축이다 — 씬을 접으면 그 안의 Beat가 전부 숨고
헤더만 남는다(`collapsedScenes`). 대본 뷰와 컷 표 양쪽에서 되고, 접힌 씬은
`3 beats` / `19 cuts`처럼 안에 무엇이 있는지 표시한다.
...
**인물은 씬의 소유물이 아니다 (2026-08-17).** 씬마다 따로 `buildSceneState`를
부르니 Scene 1의 하린과 Scene 2의 하린이 다른 카드로 나왔다. 대본은 인물을
**처음 나올 때만** 묘사하므로, 뒤 씬 대본만 읽은 모델은 근거가 없어 값을
지어낸다.

처음에는 "생성할 때 앞 씬 값을 복사"로 막았으나 그것으로는 부족했다. 복사가
끝나면 두 벌이 각자 살아서, **기준을 고쳐도 이미 복사된 씬은 따라오지
않는다.** 어느 것이 원본인지도 알 수 없다.

그래서 구조를 바꿨다 — **작품에 인물 한 벌, 씬은 달라진 것만.**

```
cast: [{ id: 'cast-하린', facts: {...}, image: <레퍼런스 한 장> }]

sceneStates: {
  scene-0: { characterIds: ['cast-하린'], characterOverrides: {} }
  scene-7: { characterIds: ['cast-하린'], characterOverrides: {
      'cast-하린': { 소지품: '노트를 손에 든 상태' }
  }}
}
```

컷 단위 `changes`가 값을 덮어쓰지 않고 구간을 얹는 것과 같은 사고방식이다.

- `resolveCharacterFacts` / `resolveSceneCharacters` — 기준 위에 그 씬의
  덮어쓰기를 얹는다. 덮어쓴 항목은 `overridden: true`를 달고 나온다.
- `resolveSceneState`가 `characters`를 **파생 필드로 채운다.** 이 값을 읽는
  곳이 스무 곳이 넘어서, 저장 구조만 바꾸고 읽는 모양은 그대로 두었다.
- `selectSceneStates`는 결과를 캐시한다 — 매번 새 객체를 만들면 zustand가
  상태가 바뀐 것으로 보고 무한히 다시 그린다.

**덮어쓰기가 기준과 같아지면 지운다.** 같은 값을 남겨 두면 나중에 기준을
고쳤을 때 그 씬만 따라오지 않는다. `setCharacterOverride`가 이것을 한다.

**쓰는 자리가 갈린다.**

| 무엇을 | 어디에 |
|---|---|
| `setSceneFact('character', …)` | **cast** — 그 인물이 나오는 모든 씬이 바뀐다 |
| `setSceneFact(…, { scoped: true })` | 그 씬의 `characterOverrides` |
| `clearCharacterOverride` | 덮어쓰기를 지우고 기준으로 되돌린다 |
| 인물 레퍼런스 그림 | 덮어쓰기가 있으면 `characterImages[id]`, 없으면 cast |

화면에서는 기준값이 그대로 보이고, 고치면 기본이 **기준 수정**이다.
`이 씬에서만 변경` 버튼으로 덮어쓰기를 만들고, 덮어쓴 항목에는
`이 씬에서 변경` 배지와 `↺ 기준으로`가 붙는다.

생성 단계에도 `known_characters`를 계속 보낸다 — 모델이 씬마다 다른 값을
내면 그것이 곧 덮어쓰기로 기록되므로, 애초에 같은 값이 나오게 하는 편이
덮어쓰기를 깨끗하게 유지한다.

레퍼런스 **그림**은 이전부터 이름으로 공유되고 있었다
(`withSharedReferences`). 이제는 cast가 그림도 함께 들고 있어 그 공유가
구조에 들어왔다.
...
**컷 플랜 단계부터 렌즈가 함께 일한다.** 지금까지 컷 플랜은 Narrative 혼자
만들었으나, 촬영 구도(`shotSize`·`angle`) 컬럼을 서사 에이전트가 정하는 것은
이상하다. 파이프라인 그림의 `에이전트 진단·공백 질의 (DG1+DG2)`가 이 자리이고,
미장센·촬영·편집이 각각 자기 몫을 세운다.

| 렌즈 | 컷 플랜에서 세우는 것 |
|---|---|
| Narrative | 컷 분해, 내용, 순서, purpose |
| 미장센 | `sceneState` (인물·공간·환경) |
| 촬영 | shotSize, angle, cameraMove |
| 편집 | 이음새 (DG2, 미구현) |

미장센·촬영·편집을 붙였다. 우측 rail(기존 Narrative Agent 패널)에 `Mise-en-scène ·
Scene state` 블록이 컷 플랜 단계에서만 나타난다. 미정(`open`) 항목은 입력칸으로
보여 그 자리에서 채우고, 채우면 `open`이 풀려 프롬프트에 들어간다.

rail을 쓴 이유: 씬 기준은 컷 표의 한 행이 아니라 표 전체를 가로지르는 것이라
컬럼으로 넣을 수 없고, 별도 단계로 두면 방금 없앤 선언 게이트와 같은 문제가
된다.

**촬영은 값을 편집하지 않고 진단한다.** `shotSize`·`angle`·`cameraMove`는 이미
컷 표의 컬럼이라 rail에서 또 편집하면 같은 값을 두 곳에서 고치게 된다. 대신
`diagnoseCoverage`가 **한 컷만 봐서는 알 수 없는 것**을 짚는다 — 같은 크기 연속,
리액션 컷 누락, 공간을 세우는 컷 없음, 점프컷 위험. 진단은 컷을 지목하고 그리로
데려다주기만 하며, 고치는 것은 표에서 한다 (발견과 처분의 분리).

**편집도 진단만 한다.** 컷 삽입·삭제는 표에 이미 있다(행마다 `+`). 그래서
`diagnoseSeams`는 컷 사이를 짚는다 — 대본의 행동 여러 개가 컷 하나에 압축된 경우,
붙어 있는 두 컷의 내용이 같은 경우, Beat가 통째로 컷 없이 넘어간 경우. 기본
예제에서 셋 다 나온다(B1·B7 압축, 6-1·6-2 중복).

**이음새는 객체가 됐다.** `useStore`의 `seams`가 컷과 컷 사이를 담는다.
`design_goal.md` DG2 P1이 말하는 넷 중 '추가 컷의 필요성'은 편집 렌즈의 진단이
이미 맡고 있으므로 셋만 둔다.

| 항목 | 묻는 것 |
|---|---|
| `join` | 어떻게 이어지나 — 컷 / 매치컷 / 디졸브 / 페이드 |
| `elapsed` | 얼마나 흘렀나 — 연속 / 잠시 / 경과 |
| `elision` | 무엇을 건너뛰었나 (자유 텍스트) |

컷의 속성이 아니라 **별도 객체**인 이유: 컷이 병합·분할되어도 사이에 무엇이
있었는지가 남아야 한다. 앞 컷의 id로 식별한다(`seamKeyFor`).

편집 자리는 **Panels의 `GridView`, 패널 사이**다. 컷 플랜 표가 아니라 여기인
이유는 이음새 문제가 그림을 이어 봐야 드러나기 때문이다. 기존 gap fill 버튼
(컷 추가) 바로 아래에 두되 자리를 나눴다 — 컷을 넣는 것과 사이를 기술하는 것은
다른 일이다.

**이음새는 진단과 프롬프트가 읽는다.** 기록만 되고 아무도 안 읽으면 칸만 늘어난
것이 된다.

- `diagnoseSeams`가 두 가지를 더 짚는다. 경과를 표시했는데 연결이 그냥 컷인
  경우(관객은 연속으로 읽는다), 생략을 적어두고 아무 표시도 하지 않은 경우
  (기록만으로는 관객에게 전달되지 않는다). 둘 다 `컷 간 관계` 층위다.
- `buildCutPrompt`가 앞 이음새를 읽어 경과가 있으면 문장을 넣는다 — "앞 컷에서
  시간이 흘렀다. 인물의 자세와 위치가 그대로일 필요는 없다." 이것이 없으면
  모델은 앞 컷의 연속으로 그린다.

컷에서 이음새를 바로 찾을 수 없다는 점에 주의. 이음새는 패널 사이에 붙으므로
컷 → 패널(`cutPlanItemId`) → 앞 패널의 이음새 순으로 찾는다(`seamBefore`).

**병합·분할이 붙었다** (DG2 P1). `컷 간 관계` 층위에 개입 수단이 없던 것이
채워졌다.

- **병합**(`mergeCuts`) — 이음새 편집기 안의 `두 컷 합치기`. 사이 이음새는 컷
  안이 되므로 사라지되, 거기 적힌 `elision`은 내용으로 옮긴다. 뒤 컷에 걸려
  있던 이음새는 앞 패널로 승계된다. 뒤 컷의 패널이 사라지므로 그림이 있으면
  먼저 확인을 받는다 (Spec §22: 조용한 파괴 금지).
- **분할**(`splitCut`) — 패널 hover의 가위 버튼. 컷 하나의 일이라 이음새
  편집기가 아니라 패널에 둔다. 새 이음새는 `컷 · 연속`이 기본이다 — 한 컷을
  쪼갠 것이므로 시간이 흐르지 않았다. 내용은 사용자가 나눈다.

둘 다 컷·패널·이음새를 함께 움직인다. 하나만 바꾸면 고아 패널이 생기거나
프롬프트가 붙지 않는다. 진단 → 개입 → 해소가 이어지는 것을 확인했다(중복 진단을
병합으로, 압축 진단을 분할로).

**실행 전 미리보기가 붙는다** (DG2 P3: "실행 전에는 영향받을 패널과 요소를
알리고"). 버튼을 누르면 바로 실행하지 않고 다음을 보여준다.

- 지금 → 바뀐 뒤 (내용 대조)
- 함께 바뀌는 것: 컷 수 변화, 이음새 생성·소멸, 사라지는 그림
- **합쳐질 내용은 그 자리에서 고친다.** 자동으로 이어붙인 문장을 기본값으로
  두되 textarea로 수정할 수 있다 — 그대로 두면 두 컷이 무엇이 되는지를 AI가
  정하게 된다.

`mergeCuts(id, { content })`로 확정된 내용을 넘긴다. 생략하면 기존처럼 자동
조립한다.

정한 것이 없는 이음새는 hover에서만 보인다(`isSeamMarked`). 전부 '컷 · 연속'인
기본값까지 상시 표시하면 그리드가 표시로 뒤덮이고, 감독이 실제로 정한 것이
묻힌다.

**진단은 문제의 층위를 밝힌다.** DG2는 원인이 속성·컷 구성·컷 관계·씬 구조
중 어디인지 진단하고 해당 층위에서 개입할 것을 요구한다. `PROBLEM_LAYERS`가 그
넷이고, Cut Plan과 패널 분석의 모든 진단이 같은 `layer` 값을 사용한다.

관점과 층위는 독립된 축이다. 미장센·촬영·편집은 무엇을 근거로 문제를 발견하는지
정하고, 층위는 어디에서 수정할지를 정한다. 따라서 세 관점 모두 네 층위를 진단할
수 있으며, 관점별 고정 규칙과 이론 근거로 진단 내용만 제한한다.

| 층위 | 예 | 개입 수단 |
|---|---|---|
| 속성 | 샷 크기와 전달 정보가 어긋남 | 표의 셀 편집 |
| 컷 구성 | 리액션 없음, 압축됨, 구간에 컷 없음 | 삽입·삭제·분할·병합 |
| 컷 관계 | 동작·시선·시간 연결이 끊김 | 이음새와 관련 컷 편집 |
| 씬 구조 | 공간·정보의 전체 전개가 어긋남 | 재배열 + `sceneState` |

층위를 표시하면 같은 문제를 무조건 패널 재생성으로 보내지 않고, 해당 개입 수단으로
바로 연결할 수 있다. 컷 관계에는 이음새, 컷 구성에는 삽입·삭제·분할·병합을 사용한다.

rail은 에이전트별로 접힌다. rail 접기(자리 비우기)와 에이전트 접기(누구를 보나)는
다른 축이라 컨트롤을 나눴고, 한 번에 하나만 펼친다. 각 헤더의 숫자는 남은 일
개수다 — Narrative는 제안 수, 미장센은 미정 항목 수, 촬영·편집은 진단 수.

**두 화면이 같은 `sceneState`를 본다.** 씬 기준을 다루는 자리가 둘이다.

- `DecisionBoard`의 미장센 렌즈 → `Scene State` — 인물 카드(사진·상세 편집),
  장소 2D 평면도까지 있는 본 편집 화면
- Storyboard 우측 rail의 `M` 에이전트 — 컷 플랜 단계의 요약과 미정 항목 입력

한쪽을 없애지 않은 이유: rail에서 미장센만 빠지면 `N / C / E`만 남아 어색하고,
Board 화면은 컷 플랜 단계에서 열려 있지 않다. 대신 **둘 다 스토어를 본다** —
어느 쪽에서 고쳐도 `buildCutPrompt`에 반영된다(양방향 확인함).

`MOCK_MISE_SCENE_STATE` 상수는 제거했다. 같은 정보가 두 벌 있으면 갈라진다.

**인물과 공간은 씬 안에서 변한다.** 값 하나로 두면 열두 컷 전부에 같은 문구가
들어간다 — 재인은 젖은 채 들어와 굳어가고, 형광등은 깜빡이다 꺼진다. 기존 툴이
"공간 하나가 끝"인 지점이 여기다.

각 `fact`가 `changes`를 갖는다: `{ at: 6, value: '젖은 채 굳어 있음' }` — 7번째
컷부터 그 값이다. `buildCutPrompt`에 `cutIndex`를 넘기면 그 시점의 값이 들어간다.

값을 덮어쓰지 않고 **구간을 얹는** 이유: 처음 값이 지워지면 앞 컷들이 무엇이었는지
알 수 없게 된다. `factTimeline(fact)`이 전체 구간을 돌려준다.

**인물·장소·환경 셋 다** 변화를 갖는다. 문이 닫히고 캐비닛이 열리고 조명이
꺼지는 것도 컷을 가로지르는 변화다. 셋이 `SceneFactChanges` 하나를 공유한다.

편집은 Board의 Scene State 안. 각 항목 아래 `+ 변화` → 어느 컷부터 무엇으로
바뀌는지 고른다. 컷을 고르는 것이므로 자유 입력이 아니라 목록이다.

#### 표현 밀도는 컷 플랜에서 정한다 (2026-08-16)

`러프 콘티 / 디테일 스케치 / 실사 프리비즈`는 Panels의 생성 바에만 있었다.
그런데 **레퍼런스는 컷 플랜에서 만들어진다** — 기준 그림이 먼저 나오고
화풍이 나중에 정해지는 역순이었다.

증상은 `reference_image.py`에 있었다. `BASE_STYLE`이 "Black-and-white
cinematic line drawing"을 하드코딩해, 실사 보드를 만들 예정이어도 인물·공간
기준은 언제나 선화로 나왔다. 그 선화가 `panel_image.py`에서 참조로 물리므로
두 화풍이 한 패널 안에서 경쟁한다 — 그 파일의 docstring이 스스로 경계한
상황이다.

- `PANEL_STYLE_PRESETS`를 `useStore`로 옮겼다. 두 단계가 같은 목록을 본다.
- `style_preset`이 `ReferenceImageRequest`에 생겼고, `PRESET_LOOKS`가 밀도별
  화풍을 정한다. 하드코딩된 `BASE_STYLE`은 없앴다.
- 선택기는 **양쪽에 둔다.** 값은 `panelStylePreset` 하나이므로 어디서 고르든
  같은 결정이다. Scene State가 두 화면에서 편집되는 것과 같은 방식이다.

**`그림체` 텍스트 칸은 없앴다.** 처음에는 "밀도 안에서 덧붙이는 지시"로
남겼으나, 같은 것을 두 곳에서 정하게 되는 문제가 그대로였다. 무엇보다 글은
앵커 이미지보다 약하다 — 레퍼런스가 `images.edit`의 입력으로 물리므로,
글과 그림이 어긋나면 글이 지는 쪽이다. 어차피 무시될 칸을 두면 감독은
정했다고 믿는데 화면은 따르지 않는다.

- `ENVIRONMENT_LABELS`에서 `그림체`를 뺐다(이제 `["시간"]`).
- 조명·분위기 지시가 갈 곳은 이미 `scenePromptNote`(장면 지시)에 있다.
- 옛 상태에 값이 남아 있어도 목록에 다시 뜨지 않게 `filter`는 남겨 두었다.

**같은 버그가 `그림체 맞추기`(restyle)에도 있었다.** `image_generator.py`의
`style_prelude(style or '')`가 preset 인자를 빼먹어 언제나 `rough`로 떨어졌다.
실사 보드에서 손으로 그린 패널만 러프 스케치로 남는다. `EnhanceSketchRequest`에는
`style_preset`이 이미 있었는데 라우트가 서비스로 넘기지 않고 있었다 — 셋 다
연결했다.

**이미 만든 레퍼런스는 자동으로 다시 그리지 않는다.** 각 레퍼런스에 만든
밀도(`stylePreset`)를 남기고, 현재 값과 다르면 카드에 `러프 콘티로 만든 기준`
배지를 띄운다. 처분은 옆의 `다시 생성`으로 감독이 한다 — 마음에 들어 하던
기준 그림이 예고 없이 사라지면 안 된다(발견과 처분의 분리).

밀도 기록이 없는 옛 레퍼런스는 배지를 띄우지 않는다. 모르는 것을 갈렸다고
말하면 멀쩡한 기준까지 다시 그리게 된다.

#### 러프 콘티는 기준 그림 없이 간다 (2026-08-17)

세 밀도를 실제로 뽑아 보고 알게 된 것: **AI는 "대충 그려라"를 못 따른다.**
프롬프트로 `stick-figure level`, `wobbly`, `almost too crude`까지 적어도
깔끔한 라인 아트가 나왔다. 반면 감독이 종이에 그린 스케치를 앵커로 물리자
한 번에 됐다 — 얼굴이 빈 타원, 겹쳐 그은 펜 선, 종이 질감까지.

**이미지가 텍스트보다 강하다**는 것이 여기서도 확인됐다. 그래서 러프 앵커는
생성물이 아니라 실제 손그림이다. detailed·photoreal은 AI 생성이 맞다 —
연필 밀도와 실사는 AI가 잘 만들고, 앵커의 목적은 출처가 아니라 밀도다.

**러프에서는 인물·공간 레퍼런스를 쓰지 않는다.**

- 얼굴이 빈 타원인 그림에 "같은 얼굴로 이어지게" 할 것이 없다.
- 오히려 그려진 기준을 물리면 그쪽으로 완성도가 끌려 올라간다.
- `referencesForCut`이 러프면 앵커만 넣는다(구조도는 유지 — 배치 고정은
  러프에서도 유효하다).
- 컷 플랜 레일에서 레퍼런스 카드를 숨기고, **레퍼런스 미비로 `컷 플랜
  확정`을 막던 게이트도 건너뛴다.** 쓰지도 않을 그림을 요구하면 안 된다.

#### 관객 읽기는 그림을 보는 일이다 (2026-08-17)

`viewer_initial_reading`이 `gpt-5.4-nano`로 돌고 있었다. 대본도 샷 라벨도
주지 않으므로 **픽셀이 유일한 근거**인데 가장 약한 모델이었고, 그래서
자세·시선·소품을 자주 놓쳤다.

연출 렌즈가 같은 이유로 이미 올린 전례가 있다 —
`DEFAULT_LENS_MODELS`의 미장센 주석: *"mini로는 시선 방향을 사건 설명에서
가져와 답하는 일이 잦았다."*

- 읽기 → `gpt-5.4` (`VIEWER_READING_MODEL`로 낮출 수 있다)
- 비교 → `gpt-5.4-mini` (텍스트 작업이지만 지적을 컷별로 갈라야 한다)
- 프롬프트에 **볼 순서**를 넣었다. 기존에는 금지어만 있고 어떻게 보라는
  말이 없었다. 러프 콘티 대응도 함께 — *빈 얼굴을 감정으로 읽지 말고
  불분명하다고 말하라*.

#### Scene State를 씬 단위 구간으로 (2026-08-17)

`sceneState`는 씬 단위 데이터인데 화면은 그렇지 않았다. 변화를 보려면 카드를
뒤집어 항목마다 들어가야 했고, 여러 인물·공간의 변화가 **같은 컷에서 함께
일어나는지** 알 수 없었다.

`spatialStagesFor`가 이미 그 구간을 계산하고 있었다(모든 fact 변화의 시작점
합집합). 2D 편집기 탭에서만 쓰던 것을 Scene State 상단으로 올렸다 — 같은
기준을 쓰므로 구간을 고르면 **값과 2D 배치가 함께** 바뀐다.

씬이 여럿일 때 세 가지가 어긋나 있었다.

- **`shots`가 브랜치 전체였다.** `sceneState`는 한 씬 기준인데 구간은 모든
  씬의 컷으로 계산돼, `S1–S19`처럼 씬을 넘는 구간이 나왔다 → `sceneShots`로
  활성 씬만 넘긴다.
- **라벨이 배열 첨자였다.** 씬으로 걸러진 목록은 첨자가 0부터 다시 시작해
  Scene 2의 첫 구간이 `S1`로 나왔다(실제로는 S16) → `numberFrom`으로 보정.
  숫자를 박지 않고 그 씬 첫 컷의 전체 위치에서 계산한다.
- **`selectLayoutForCut`은 전체 shots로 단계를 만들고 있었다.** 보드는 씬
  범위로 만드니 `stage.id`가 서로 달라, 씬 2에서 단계별로 그린 도면을 생성이
  찾지 못했다 → 생성 쪽도 같은 씬 범위로 맞췄다.

**씬 선택을 이 화면에 넣었다.** 전에는 컷 플랜 레일로 나가 Beat를 옮겨야
다른 씬이 보였다 — 씬 단위 화면인데 씬을 고를 자리가 없었다.

#### 인물 항목을 넷으로 줄였다 (2026-08-17)

다섯 항목이 각자 `+ 변화` 버튼을 갖고 있어 화면이 상자로 뒤덮였다. 성격에
따라 두 갈래로 나눴다.

| | 항목 | 변화 |
|---|---|---|
| 생김새 | 성별·나이 / 외형 기준 / 체형 | 걸 수 없다 |
| 상태 | 상태 (`기본 태도`+`소지품` 통합) | 걸 수 있다 |

프롬프트에는 원래부터 라벨 없이 값만 이어붙으므로(`settledFacts`) 합쳐도
생성 결과는 달라지지 않는다. 변화 버튼은 `상태`·`시간`·`고정 소품`에만 둔다.

**인물 카드는 편집 중에 draft를 그린다.** `addFactChange`가 스토어(`cast`)에만
쓰니 화면에 안 나타나고, 이어서 Save를 누르면 draft가 스토어를 덮어써 방금
넣은 변화가 사라졌다 → draft에도 함께 쓴다. 삭제도 마찬가지다.

#### 이음새 삽입이 패널을 만들지 않았다 (2026-08-17)

`splitCut`·`mergeCuts`·`swapCutsAtSeam`·`deleteCut`은 컷과 패널을 함께
움직이는데, **삽입만 컷 플랜에만 넣고 있었다.** 그래서 `넣기`를 눌러도 화면에
아무 변화가 없고, 프롬프트도 붙을 자리가 없었다(`cutPlanItemId`로 짝짓는다).

경로가 둘 다 깨져 있었다 — 편집 렌즈의 `acceptCutInsert`와 패널 그리드의
`addCutPlanItem`.

`addCutPlanItem`은 컷 플랜 표(확정 전, 패널이 아직 없음)에서도 부르므로,
**패널이 이미 컷에 붙어 있을 때만** 패널을 만든다. 확정 전에는 컷만 넣고
`Accept cut plan`이 한 번에 맞춘다.

`removeCutPlanItem`·`moveCutPlanItem`이 패널을 안 건드리는 것은 의도다 —
표 전용이고, 패널 쪽에는 `deleteCut`·`swapCutsAtSeam`이 따로 있다.

#### 진단 층위 카드가 열리지 않았다 (2026-08-17)

`<details open={계산값}>`은 React 제어 컴포넌트라 브라우저가 토글해도 다음
렌더에서 되돌아간다. `keep` 카드는 열 수 없고 `change` 카드는 닫을 수 없었다.
요약 문장은 `white-space: nowrap`으로 한 줄에 고정돼 있어, 못 열리니 잘린
글자를 볼 방법이 없었다.

열림 상태를 state로 들고(처음 상태만 판정에서 정한다), 펴면 요약이 여러 줄로
풀리게 했다. `defaultOpen`은 React가 `<details>`에서 지원하지 않는다.
...
### 6.6 Decision Board와 Creative Lens

Decision/Lens 화면은 상위 Narrative와 세 하위 Creative Lens의 역할을 UI
흐름으로 시험한다. 작업 공간의 대안 카드 일부와 canonical Decision 데이터는
여전히 Mock이지만, 패널에 대한 미장센·촬영·편집 진단은 실제 Agent API에 연결됐다.

2026-08-11 기준 실제 진단은 관점별 고정 규칙 4개와 규칙당 검토된 이론 2개를
사용한다. 전체 연결표는 [`DIRECTING_RULES.md`](./DIRECTING_RULES.md)에 있다.
에이전트는 0~1개의 핵심 진단만 반환하며, 선택한 `rule_id`에 연결되지 않은 이론은
서버에서 거부한다. 패널의 카메라 화살표와 메모도 `directing_notes`로 전달하므로,
카메라 움직임은 선언이 있을 때만 분석할 수 있다.
...
- 첫 화면을 `Story`로 바꾸고 대본을 빈 상태에서 시작한다.
...
### Cut Plan 관점별 규칙 보완 — 후속 작업 (2026-08-10)

지금은 패널 생성 후의 연출 분석을 먼저 검증한다. Cut Plan 규칙 보완은 삭제한
것이 아니라 **의도적으로 후순위로 둔 작업**이다.

현재 Cut Plan에는 촬영의 `diagnoseCoverage`와 편집의 `diagnoseSeams`가 일부
규칙 기반 진단을 제공한다. 미장센 규칙은 없고, 생성 후 패널 분석에서 사용하는
고정 규칙 ID 체계와도 아직 분리돼 있다. 단, 진단 층위는
`attribute / shot_structure / shot_relation / scene_structure`로 통일됐다.

후속 보완 범위:

1. Cut Plan의 미장센 규칙을 추가한다.
2. 기존 촬영·편집 규칙을 에이전트별 규칙 목록으로 정리한다.
3. Cut Plan(생성 전)과 패널 분석(생성 후)의 규칙 ID를 통일한다.
4. 생성 전 계획과 실제 이미지 진단을 비교해, 계획이 화면에서 지켜졌는지 표시한다.

역할은 구분한다. Cut Plan 규칙은 텍스트·컷 목적·샷 설정·이음새를 점검하고,
패널 규칙은 실제 이미지의 시각적 근거를 점검한다. 같은 문제를 두 화면에서
중복 제안하지 않는다.
...
### 연출 규칙 이미지 회귀 테스트 (2026-08-11)

- 저장 예시 10개로 미장센·촬영·편집 고정 규칙 12개의 발동/비발동 총 24개
  실제 비전 호출 사례를 만들었다.
- 최종 동일 실행에서 24/24가 통과했다. 발동 사례는 규칙 ID뿐 아니라 진단 층위,
  화면 근거, 수정 방향, 연결 이론 1개, 질문 수까지 검증한다.
- 기본 모델은 미장센 `gpt-5.4-mini`, 촬영 `gpt-5.4`, 편집 `gpt-5.4`다.
  촬영 mini는 대치축 판정이 흔들려 상위 모델로 올렸다.
- 실행기는 `backend/app/tools/test_directing_rules.py`, 상세 결과와 한계는
  [`DIRECTING_RULE_TEST_REPORT.md`](./DIRECTING_RULE_TEST_REPORT.md)에 정리했다.

---

### 검토 결과에 기준·선택지·관계 붙이기 (2026-08-13)

논문 4.3의 세 항목(Criterion / Alternative / Cross-lens relation)을 채웠다.

**Criterion — 새로 만들지 않았다.** 규칙 12개가 이미 `trigger`(이럴 때
문제) / `reject_when`(이러면 문제 아님)을 갖고 있고, 그 쌍이 곧 기준이다.
다만 기계 조건으로 쓰여 있어 감독이 읽을 한 문장으로 옮겼다. 진단에는
서버가 `rule_id`로 붙인다 — 모델이 쓰게 하면 같은 문제에 매번 다른 잣대가
적용되고, 근거도 규칙에 달린 이론 출처를 벗어난다.

**Alternative — `그대로 두기`가 언제나 첫 번째다.** 유지도 연출 결정이고,
선택지에 없으면 진단이 곧 지시가 된다. 모델이 빠뜨리면 스키마 validator가
만들어 앞에 넣는다. 나머지는 서로 배타적이어야 한다 — 함께 할 수 있는
것을 나열하면 선택이 아니라 조언 목록이다.

**Cross-lens — `multi` 모드가 501 스텁이었다.** 화면의 `공통 판단`과
`연출 선택`은 하드코딩된 문장이었다. 세 렌즈를 병렬로 돌리고, 그 결과만
2차 호출로 넘겨 관계를 찾는다. 관계에 방향이 있다(`camera → editing`) —
원인 렌즈를 고쳐야 하고, 영향받은 쪽을 고치면 증상만 사라진다.
`먼저 볼 곳`이 그 방향에서 나온다.

**감독 판정이 다음 분석에 반영된다.** 관계에 답하면(`의도한 거야` 등)
그 판정이 `settled`로 함께 전송되고, AI가 같은 지적을 반복하지 않는다.
관계 종류마다 묻는 것이 다르다 — consequence는 맞나 틀리나, conflict는
무엇을 우선하나.

**라우팅을 규칙별로 바꿨다.** 이전에는 `level` 하나로만 갈려서, 미장센
배치 문제가 컷 플랜 표로 갔다 — 거기엔 배치를 고칠 것이 없다. 규칙 12개
각각이 목적지를 갖는다:

| 렌즈 | 어디로 |
|---|---|
| 미장센 배치·외형 | 패널 프롬프트 + 직접 그리기 |
| 미장센 공간 일관성 | 2D 구조도 |
| 촬영 샷 값 | 컷 플랜 표 |
| 촬영 움직임 | 카메라 화살표 |
| 편집 전부 | **이음새** (합치기·나누기 미리보기가 거기 있다) |

편집을 컷 플랜으로 되돌리지 않는 이유: 그림을 다 그려 놓고 텍스트 단계로
가면 무엇을 건드릴지 알 수 없고, 컷 플랜 표에는 그림이 없다. 이음새에는
이미 DG2 P3가 구현돼 있다 — 합치기를 누르면 무엇이 사라지는지 먼저 보인다.

**시간: 71초 → 22초 + 3초.** 관계를 버튼으로 분리했다. 2차 호출은 이미지를
다시 보내지 않고 렌즈들이 쓴 판단만 넘긴다.

**분석 결과에 지문을 남긴다.** 검토한 패널(그림 + 샷 크기)의 지문을 저장해
두고, 고치고 돌아오면 `지난 결과입니다`로 표시한다. 이전에는 의도를 고칠
때만 stale이 됐고, 정작 그림을 고쳐도 옛 분석이 최신인 것처럼 남았다.

**아직 안 한 것**
- `narrative에게 물어보기` — 화면을 옮기지 않고 제안만 받는 것
- 다관점에서 `→ 촬영에서 이어서 보기` 링크 (판정 버튼과 분리)
- 관객 읽기 조건의 장면 간 유지 방식

### 관객 읽기 조건 (2026-08-18 갱신)

교수 피드백: 페르소나가 필수인데 어떻게 부여할 것인가. 지금 관객은
"누구도 아닌 관객"이라 감독이 반박할 수 없고, 관객 의견이 판정 대상이
아니라 판결이 된다.

**정해진 방향**
- 세 기본 조건은 실제 인구 집단을 흉내 내는 페르소나가 아니라, 같은 패널
  시퀀스에서 무엇을 먼저 볼지 달리하는 attention condition이다.
- 처음 보는 관객: 사전 정보 없이 누가 누구고 무슨 일이 벌어지는지 따라간다
  (`이해 가능성`).
- 영화에 익숙한 관객: 프레이밍, 반복, 생략, 컷의 관계가 만드는 기대와
  강조를 따라간다 (`영화적 표현에 대한 민감성`).
- 이야기 흐름을 중요하게 보는 관객: 컷 사이 사건·인과관계·정보 변화가 어떻게
  이어지는지 따라간다 (`sequential / causal coherence`).
- UI에서는 각 카드의 설명을 `먼저 보는 것`으로 제시한다. 세 번째 문구는
  `컷 사이에서 사건과 정보가 어떻게 이어지는지 살핍니다.`이다.
- 내부 id `context_close`는 기존 결과와의 호환성을 위해 유지하지만, label과
  모델 instruction은 이야기 흐름 기준으로 바꾼다.
- 사용자는 필요하면 별도의 관객 시선을 직접 추가할 수 있다.
- 평론가·프로그래머는 뺀다. `완성도`나 `문화적 의미`로 흘러 순차 읽기의
  목표(이 순간 무엇이 읽혔나)와 멀어진다.

**버린 축**
- `지키고 싶은 경험`(낯섦/접근성/…) — 작품 전체의 성격이라 패널 몇 장에
  걸리지 않는다. 골라도 결과가 안 바뀐다.
- `허용 가능한 모호함`(낮음/중간/높음) — 고를 기준이 없고, 무엇에 대한
  모호함인지 불분명하다.
- 두 축은 사실 같은 질문이었다: "안 읽혀도 되는 게 뭐고 반드시 읽혀야
  하는 게 뭔가?" 그건 미리 묻지 않고 **나온 지적마다 판정**한다.

**미해결 — `issue_kind` 여섯 개의 근거가 없다.** 이론이나 선행연구가 아니라
네 에이전트에서 역산된 것이다. "관객이 겪는 이해 실패의 유형"이 아니라
"우리 시스템의 어느 에이전트가 고칠 수 있나"로 나눈 것. 후보:
- 관객은 분류하지 않고 증상만 말하고, 분류는 시스템이 한다 (실제 관객은
  "이건 cut_connection 문제입니다"라고 말하지 않는다)
- 이론에서 다시 가져온다 (Bordwell narration, Zacks event segmentation)

**정하지 않은 것**
- 관객을 몇 명 세울 것인가 (한 명이면 지금 구조에 조건만 얹으면 되고,
  여럿이면 "서로 다르게 읽었다"는 사실 자체가 정보가 된다)
- 관객이 그림을 잘못 읽었을 때(`24H`를 `24:11`로) 전달 실패로 볼지
  관객 모델의 한계로 볼지 — 실험 설계에 걸린다
...
- 브랜치: `feature/scene-references` (`feature/scene-flow-fill`에서 분기)
- 원격: 아직 push 안 함
- 최근 기능 커밋:
