...
그래서 컷에 남은 축은 `provenance`(AI/User) 하나다. 출처는 데이터와 표의
미세한 왼쪽 선에 남기되, 푸터는 AI 컷 전체가 아니라 에이전트가 짚었거나 샷이
미정인 `확인할 것 N`만 센다. 프롬프트만 고친 경우에는 출처가 바뀌지 않는 기존
가드가 그대로 유지된다.
...
**세 화면이 같은 `sceneState`를 본다.** 씬 기준을 다루는 자리가 셋이다.

- `DecisionBoard` 상단의 `장면 기준` → 인물·공간·장면 공통 값을 짧게 확인하고
  **그 자리에서 고치는** 화면. 다른 검토와 같은 범위 바 높이를 유지해 전환할 때
  화면이 출렁이지 않는다. 인물은 위 전체 폭에 최대 세 명까지만 먼저 두고, 더
  많으면 펼친다. 공간·장면 공통은 아래 두 카드다.
- `DecisionBoard`의 미장센 렌즈 → `Scene State` — 인물 카드의 사진 교체, 항목별
  컷 변화, 장소 2D 평면도처럼 더 깊은 편집이 필요한 작업대다.
...
| | 항목 | 변화 |
|---|---|---|
| 생김새 | 성별·나이 / 외형 기준 | 걸 수 없다 |
| 상태 | 상태 (`기본 태도`+`소지품` 통합) | 걸 수 있다 |

`체형`은 별도 입력을 없애고 머리·옷차림·실루엣을 함께 담는 `외형 기준`에
합쳤다. 레퍼런스 한 장이 이미 이 정보를 함께 고정하므로 입력칸을 나눌 이유가
없었다.
...
- Narrative 요청과 Decision Board의 미장센·촬영·편집·다관점 요청은 작업
  세션 동안 칸별로 최근 50개를 기억한다. 첫 줄에서 `↑`/`↓`를 누르면 이전
  요청을 순회하고, 탐색 전 작성 중이던 초안으로도 돌아온다.
...
**초보자 검토 흐름을 정리했다 (2026-08-25).** 상단에는 `연출 검토 / 읽힘 검토 /
장면 기준`을 둔다. 앞의 둘은 컷을 판단하는 검토이고, `장면 기준`은 인물·공간·
환경처럼 이후 패널 생성에 계속 쓰이는 기준을 확인·수정하는 별도 문맥이다. 미장센·
촬영·편집은 없어지는 기능이 아니라 `전체 분석 보기` 안의 상세 렌즈로 내려간다.

- Decision Board에 들어오면 연출 분석을 먼저 시작하고, 렌즈별 보고서 대신
  실제로 답할 진단을 `확인할 것 N개`에서 한 건씩 보여준다.
- 진단이 둘 이상이면 번호·렌즈가 적힌 작은 전환 버튼을 함께 보여준다. 한
  건씩 보는 구조는 유지하지만, 다음 진단이 숨겨진 것으로 보이지 않게 한다.
- 한 진단의 기본 선택은 `현재 유지 / 추천안 보기 / 직접 수정`이다. 추천은
  같은 카드 안에서 현재 상태와 변경 효과를 비교하고, 직접 수정할 때만 해당
  렌즈의 상세 도구로 이동한다.
- `전체 분석 보기`에서 미장센·촬영·편집 보고서를 세 열로 반복하지 않는다.
  렌즈별 `열기`, `먼저 볼 곳`, 관계 판정 버튼도 제거했다. 이 영역에는
  감독만 답할 수 있는 확인 질문과 자동 분석된 렌즈 간 관계만 짧게 남긴다.
  둘 다 없으면 `전체 분석 보기` 자체를 표시하지 않는다. 상세 수정 도구는
  메인 진단의 `직접 수정`을 통해서만 연다.
- 다시 분석한 결과는 새 검토로 취급한다. 이전 실행에서 `현재 유지`한 진단이
  같은 id로 돌아와도 자동으로 숨기지 않는다.
- 읽힘 검토는 결과를 보기 전에 사람 유형을 고르게 하지 않는다. 첫 화면은
  `화면만으로 읽어보기` 한 가지 동작만 보여주며, 첫 결과가 나온 뒤에만
  연출 방식·컷 연결처럼 무엇을 먼저 살필지 정하는 읽기 조건을 추가로 연다.

**검토 범위는 패널에서 직접 고른다 (2026-08-24).** `한 컷 / 범위 / 현재
선택으로 변경`과 시작·끝 드롭다운을 제거했다. 상단에는 확정된 범위와
`범위 바꾸기`만 보인다. 바꾸기를 누른 뒤 왼쪽에서 첫 컷과 끝 컷을 누르면
사이의 연속 컷이 함께 강조되고, 확정하기 전까지 실제 분석 범위는 바뀌지
않는다. 한 컷만 누른 채 확정하면 단일 컷 검토가 된다. 이미지가 없는 컷과
그 컷을 사이에 둔 범위는 선택할 수 없다.

관객 검토의 첫 범위는 현재 컷 주변 두 장이 아니라 `S1부터 이미지가 연속으로
생성된 마지막 컷`까지다. 중간에 미생성 컷이 있으면 그 직전에서 멈춘다.
...
**에이전트의 첫 점검은 proactive하게 시작한다 (2026-08-24).** Script가
씬·Beat 구조를 갖춘 직후 Narrative 점검을 한 번 자동 실행한다. Cut plan도
기존처럼 초안 진입 시 구성 점검을 자동 실행한다. 타이핑마다 재호출하지는
않는다. 대본이 바뀌면 이전 결과에 `변경됨 · 다시 점검`을 표시하고, 버튼은
최초 실행이 아니라 명시적인 재검사에만 쓴다.
...
- 값은 표에서 바로 수정한다. 별도 `수정` 모드는 한 클릭만 늘리므로 두지
  않는다. 대신 에이전트가 짚었거나 샷이 미정인 행에만 `확인` 표시를 붙여
  예외부터 볼 수 있게 한다. 반복되는 `AI` 배지는 표에서 숨기고 출처는
  데이터에만 보존한다.
- 첫 예외가 생기면 해당 에이전트 하나를 자동으로 열고 첫 컷으로 이동한다.
  사용자가 닫거나 다른 에이전트를 선택한 뒤에는 같은 플랜에서 다시 자동으로
  전환하지 않는다.
- 컷을 고쳐도 상단 점검 버튼 전체를 `변경됨`으로 만들지 않는다. 그 컷을
  가리키는 진단 카드만 `수정됨 · 재점검 필요`로 바꾸고, 오래된 카드의 자동
  조치 버튼은 숨긴다. 다른 컷을 가리키는 진단은 그대로 유효하다.
...
- 화면만으로 읽기: 누가 누구고 무슨 일이 벌어지는지 화면에 보이는 것만으로 따라간다
  (`이해 가능성`).
- 연출 방식에 주목: 프레이밍, 반복, 생략, 컷의 관계가 만드는 기대와
  강조를 따라간다 (`영화적 표현에 대한 민감성`).
- 컷 연결에 주목: 컷 사이 사건·인과관계·정보 변화가 어떻게
...
- 사용자는 필요하면 별도의 읽기 조건을 직접 추가할 수 있다.
