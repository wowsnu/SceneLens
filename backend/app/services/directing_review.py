...
컷 사이 연속성은 사건 설명을 믿지 말고 그림에서 직접 확인하세요. 같은 장소가 이어지는
두 컷에서, 같은 인물의 화면 좌우 위치·시선 방향·이동 방향, 그리고 문·창·복도 같은
출입구의 방향을 패널마다 나란히 비교하세요. 이것들이 뒤집혔는데 중립 컷·축 이동·공간
재정립이 그 사이에 없으면, 같은 자리의 연속으로 읽히지 않는 이음새 문제입니다. 원인이
촬영축에 있더라도, 컷 순서 변경·중립 컷 삽입·S8 좌우 반전·이음새 조정으로 관객의 이해를
이을 수 있으면 editing-cut-continuity로 진단하세요 — 촬영 렌즈가 같은 자리를 짚어도
됩니다. 두 렌즈의 처방은 다릅니다(촬영은 카메라 재배치, 편집은 컷 조작).
                ...
                *(
                    [{
                        ...
                        "image_url": {
                            ...
                            # 요청 본문은 브라우저에서 축소하지만, 판단 품질은
                            # 그림 종류와 무관하게 유지한다. detail 값은 Render
                            # 메모리 절감책이 아니므로 여기서 품질을 낮추지 않는다.
                            "detail": "high",
                        },
                    }]
                    if panel.image
