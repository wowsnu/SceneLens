...
- **원문이 따옴표('…' 또는 "…")로 적은 문구는 그 문구가 이야기의 정보입니다**
  (전광판·표지판·간판·쪽지·화면의 내용). 이런 문구는 요약하거나 `안내 문구`,
  `경고 표시` 같은 말로 바꾸지 말고, 해당 line의 text 안에 **따옴표째 원문 그대로**
  넣으세요. 그 문구가 어디에 적혀 있는지(전광판/버스 옆면/쪽지)도 함께 쓰세요.
  ✓ text: `전광판에 '막차 1분 후 도착'이 표시된다.`
  ✗ text: `전광판에 막차 도착 안내가 뜬다.`  ← 문구가 사라졌다
  ✓ text: `버스 옆면에 '현금 불가, 실물 카드 또는 모바일 태그 전용' 스티커가 붙어 있다.`
  ✗ text: `버스 옆면의 안내 문구가 보인다.`  ← 문구가 사라졌다
  이 text가 그대로 그림 프롬프트가 되므로, 여기서 빠지면 그 글자는 패널에 그려지지 않습니다.
  따옴표 없는 숫자·시각·핵심 표현도 원문 그대로 유지하세요.
