"""촬영 이론서에서 뽑은 원칙. 촬영이 하는 여러 일이 같은 근거를 쓴다.

샷을 처음 정할 때(shot_design)와 진단을 받아 고칠 때(shot_fix)가 서로
다른 기준을 쓰면, 고친 결과가 원래 설계와 어긋난다.

출처: The Filmmaker's Eye, Grammar of the Film Language,
The Five C's of Cinematography (backend/app/db/theory_texts.json).
"""
...
PRINCIPLES = """다음은 촬영 이론서에서 뽑은 원칙입니다. 이것을 근거로 정하세요.
