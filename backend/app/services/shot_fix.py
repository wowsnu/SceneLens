...
RESPONSE_SCHEMA = {
    ...
    "schema": {
        ...
        "properties": {
            ...
            # 이 처방이 무엇을 하는지 한 줄. 지금 화면에는 띄우지 않는다 —
            # 각 edit의 reason과 같은 말이 되어 두 번 읽게 됐다.
            # strict schema의 required라 필드 자체는 남긴다.
            "summary": {"type": "string"},
        },
    },
}
...
reason은 왜 이 크기인지 **짧은 한 문장**으로, 한국어로 씁니다. **그 컷이
무엇을 보여주려는지를 근거로** 쓰세요 — "차이를 내려고"가 아니라 "여기서
A의 망설임이 읽혀야 하므로"처럼 씁니다.

reason에 다음은 넣지 마세요. 화면이 이미 말하고 있거나, 사용자가 방금
읽은 것을 되풀이하는 말입니다.
- 컷 번호와 바뀌는 크기 ("컷 9를 Medium으로") — 카드에 `Wide → Medium`으로
  이미 나옵니다.
- 진단이 무엇이었는지 다시 설명하는 말 — 바로 위에 그 지적이 있습니다.
- 대본 내용을 길게 되짚는 말.

좋은 예: "재인의 시선 이동이 읽혀야 합니다."
나쁜 예: "컷 9는 재인의 시선이 리모컨에서 모니터를 지나 잠긴 캐비닛으로
이동하며 상황 전환을 보여주려는 컷이므로, Wide보다 Medium으로 좁혀야
합니다."

summary는 이 처방이 무엇을 하는지 한 줄로, 한국어로 씁니다.
예: "컷 3을 Close-Up으로 좁힘"
