    ...
    "editing": (
        ...
        DirectingRule(
            ...
            trigger=(
                ...
                "없이 충돌해 이음새에서 관객의 이해가 끊긴다. 같은 장소가 이어지는 두 "
                "컷에서 화면 방향·출입구 방향·같은 인물의 좌우 위치가 뒤집혀 같은 자리의 "
                "연속으로 읽히지 않는 경우를 포함한다 — 원인이 촬영축이더라도, 컷 순서 "
                "변경·중립 컷 삽입·이음새 조정으로 관객의 이해를 이을 수 있으면 여기서 "
                "진단한다. 촬영 렌즈가 같은 자리를 짚어도 무방하다. 처방이 다르다."
            ),
            reject_when=(
                "의도적 생략·점프·불연속이거나, 배치·촬영축을 바꾸는 것 외에는 어떤 편집 "
                "동작으로도(순서·삽입·삭제·이음새 조정) 이 이음새의 이해를 이을 수 없는 경우."
            ),
            theory_refs=(
