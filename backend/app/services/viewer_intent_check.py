...
**대조의 기준은 하나입니다: 감독이 이 컷에서 의도한 읽힘이 관객의 주된 읽힘인가.**
...
- 관객이 말한 **행동·확인·대상**이 목적의 상태나 감정을 보여 주는 구체적인
  근거라면 `reached`입니다. 그 행동을 목적과 경쟁하는 별도 해석으로 쪼개지 마세요.
  ✓ 목적 `망설임` / 읽힘 `문 안에 사람이 있는지 확인하며 들어갈지 고민한다` → reached
    (상대 확인은 들어갈지 고민하는 망설임의 행동적 표현이다.)
  ✓ 목적 `경계` / 읽힘 `문 앞에서 안을 살피며 발을 멈춘다` → reached
...
행동의 이유나 방법이 더 구체적으로 보였다는 이유만으로는 낮추지 마세요.
...
컷 플랜을 다시 찾아봐야 합니다.
- intended — 감독이 의도한 것. 목적을 그대로 옮기지 말고 이 컷에서 무엇이
